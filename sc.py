    ...
    if "." in filename:
        ...
        for line in lines:
            if filename == line.split(".")[0]:
                if file is not None:
                    raise Exception(
                        "Files with duplicated names. A file extension will be needed"
                    )
                file = line
...
def build_colored_output(start, middle, end):
    ...
    if len(end) > WRAP_LIMIT:
        ...
        while wrap_follower < initial_end_size:
            following_end += (
                " " * (len(middle) + calc_mid_section - 1)
                + end[wrap_follower : wrap_follower + WRAP_LIMIT]
                + "\n"
            )
            wrap_follower += WRAP_LIMIT
...
def cat_shortcuts_liner(filename):  # color only `` strings
    path = subprocess.check_output(["echo $SHORTCUTS_DIR_PATH"], shell=True).decode(
        "utf-8"
    )
    ...
    if "/" in filename:
        ...
        file_dir = "{}/{}".format(
            path[0:-1], check_correct_file(filename)
        )  # .replace("\\n", '')
    ...
    for line in lines:
        ...
        if index_of_first_md_char != -1:
            shortcut = line[
                index_of_first_md_char : index_of_second_md_char + 1
            ].strip()
            beginning_of_line = line[0:index_of_first_md_char].strip()
            ending_of_line = line[index_of_second_md_char + 1 :].strip()

            final_output += (
                build_colored_output(beginning_of_line, shortcut, ending_of_line) + "\n"
            )
...
# terminal shortcuts
parser.add_argument(
    "-t", "--terminal", action="store_true", help="Display terminal shortcuts"
)

parser.add_argument(
    "-c", "--pycharm", action="store_true", help="Display pycharm shortcuts"
)

parser.add_argument("-r", "--read", help="Read shortcut file from default directory")
...
if args.path:
    if not os.path.isdir(args.path):
        ...
        command = "echo \"export SHORTCUTS_DIR_PATH='{}'\n$(cat $HOME/.{})\" > $HOME/.{}".format(
            path, SHELL_PROFILE, SHELL_PROFILE
        )  # :))))))
